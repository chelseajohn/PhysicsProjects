...
for g in progress(range(n)):   
    ...
    '''
    if(g%20==0):
        create_picture(np.array(pos),int(g/20))
    '''
create_picture(np.array(pos),1)
plt.show()
...
v=np.linspace(0,max(v_list)+1,100)
maxwell=v*np.exp(-v*v/(2*T_target))/T_target
plt.plot(v,maxwell)   
plt.hist(np.array(v_list),50,normed=1)
plt.show()
plt.plot(t,Inst_press,label="pressure")
plt.legend()
plt.show()
plt.plot(t,Inst_temp,label="temperature")
plt.legend()
plt.show()
